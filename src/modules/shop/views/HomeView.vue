...
const { data: products = [] } = useQuery({
	queryKey: ['products', { page:page }],  // crea un único identificador de la query lo que se ponga dentro de []
	queryFn: () => getProductsAction(page.value),  // es la función que se llama cuando se solicita la petición desde el composable
	//staleTime: 1000 * 60  // se puede pedir que durante este tiempo. cualquier peticion que se haga va a regresar la misma info de caché.
})
...
// efect que carga la próxima pagina cuando esta mostrando la otra
watchEffect(()=>{
